...
import { betweenFormatIsoDateToString, formatIsoDate } from "../momentEx";
...
export const DateRangePickerView = observer(
  (props: {
    ...
    const [model] = useState(() => new DateRangePickerModel(props.startDate, props.endDate));
    ...
    useEffect(() => {
      ...
      function handleClickOutside(event: MouseEvent) {
        if (pickerRef.current && !pickerRef.current.contains(event.target as HTMLDivElement)) {
    ...
    const printValue =
      !isNil(model.value.startDate) && !isNil(model.value.endDate)
        ? betweenFormatIsoDateToString(moment(model.value.startDate), moment(model.value.endDate))
        : props.placeholder ?? "";
    return (
      <div ref={pickerRef} className={props.className}>
        <div style={{ position: "relative" }}>
          <Button
            ...
            className={classNames("form-control text-start form-control-button", {
              "is-invalid-custom": props.isInvalid ?? false,
            })}
            disabled={props.isDisabled}
            onClick={() => model.onClick(props.startDate, props.endDate)}
          >
              ...
              <Row className="pb-2 me-1 ms-1 justify-content-between">
                ...
                <Col md="auto" sm="auto" xs="auto">
                  <Button
                    ...
                    onClick={() => model.onCancel(props.startDate, props.endDate)}
                  >
                    Cancel
                  </Button>
                  <Button
...
class DateRangePickerModel {
  ...
  readonly onSave = (onChange: (startDate: Moment | null, endDate: Moment | null) => void) => {
    onChange(
  ...
  readonly onClear = (onChange: (startDate: Moment | null, endDate: Moment | null) => void) => {
      ...
      <div ref={pickerRef} className={props.className}>
        <div style={{ position: "relative" }}>
          <Button
            ...
            disabled={props.isDisabled}
            onClick={() => model.onClick(props.date)}
          >
            <i className="mdi mdi-calendar me-1" /> {printValue}
          </Button>
        </div>
